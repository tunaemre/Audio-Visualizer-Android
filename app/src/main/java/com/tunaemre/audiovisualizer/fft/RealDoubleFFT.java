...
*/
public class RealDoubleFFT extends RealDoubleFFTMixed
{
/**
...
*/
     public void ft(double x[], Complex1D y)
     {
         ...
         if(ndim%2 == 0) 
         {
             y.real = new double[ndim/2 + 1];
             y.imaginary = new double[ndim/2 + 1];
         }
         else
         {
             y.real = new double[(ndim+1)/2];
             y.imaginary = new double[(ndim+1)/2];
         }
         ...
         if(ndim%2 == 0)
         {
             y.real[ndim/2] = x[ndim-1];
             y.imaginary[ndim/2] = 0.0D;
         }
...
/**
  ...
  *
  * @param x an array which contains the sequence to be transformed. After FFT,
  * <em>real</em> contains the transform coeffients. Also see the comments of <em>ft</em>(double[])
...
/**
  ...
  * between <em>real</em> and complex FFT coeffients.
*/
     public void bt(Complex1D x, double y[])
     {
         if(ndim%2 == 0)
         {
             if(x.real.length != ndim/2+1)
                 throw new IllegalArgumentException("The length of data can not match that of the wavetable");
         }
         else
         {
             if(x.real.length != (ndim+1)/2)
                 throw new IllegalArgumentException("The length of data can not match that of the wavetable");
         }
